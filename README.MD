# Spartan-WHIR Solidity Verifier

This README is the canonical root document for the Solidity verifier work in this workspace. It contains the reviewed implementation guide, locked decisions, implementation order, source-of-truth boundaries, and the current branch state.

## Stage Status

| Stage                                                                                                                                    | Status    |
| ---------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| Stage 0: Freeze ABI schema + Foundry project + Rust fixture/export tooling                                                               | completed |
| Stage 1: KoalaBear base field + quartic/octic extension arithmetic (quartic-first, octic-ready)                                          | completed |
| Stage 2: Keccak challenger / transcript parity, driven by exported Rust traces                                                           | completed |
| Stage 3: Merkle multiproof verification with domain-prefixed Keccak and digest masking                                                   | completed |
| Stage 4: Standalone WHIR verifier -- quartic passes first, then octic, with the main WHIR optimization pass and the standalone blob path | pending   |
| Stage 5: Full Spartan verifier (outer/inner sumcheck + circuit-specific generated constants)                                             | pending   |
| Stage 6: Spartan-specific and deployment-path optimization with A/B comparisons                                                          | pending   |
| Stage 7: Full-Spartan blob wrapper + calldata benchmarking                                                                               | pending   |

### Quintic Trinomial Verifier Status

#### Current state as of 2026-05-05

We are building a quintic standalone-WHIR verifier over
`QuinticTrinomialExtensionField<KoalaBear>` using `X^5 + X^2 - 1`.
...
- native blob verifier gas: `5,526,933` as measured by Foundry, and
  `5,646,080` total transaction gas on Anvil. Of that transaction gas,
  `4,768,744` is pure execution gas (the `21,000` transaction base cost and
  the calldata cost have been subtracted).

The native blob verifier is the canonical Solidity target. The typed ABI
verifier and the decoded blob wrapper remain parity and debugging paths.
The previous quintic build target, `k22_jb100_ext5_lir4_ff4_rsv4`, is still
checked in for comparison. It used `pow_bits = 27`, measured
`133.516761291s` prover time, and measured `6,447,048` total native
transaction gas on Anvil. The current `rsv3_pow28` target reduces native
transaction gas by about `12.4%` and increases measured prover time by about
`105%`.

The field invariant that matters for Solidity is the transcript image: five
coefficient-order little-endian `u32` values, exactly 20 bytes. Schedule-search
notes and commands are in `sol-spartan-whir/README.md`.
...
The goal of this document is to build a **Solidity verifier** for this proof system, deployable on Ethereum-compatible EVM chains. The verifier must reproduce the exact Fiat-Shamir transcript and verification logic of the Rust implementation.
...
- `./whir-old/` -- Older Rust WHIR implementation (BN254-based). Only useful as a workflow reference for fixture export. Not a schema or logic reference.
...
**Use `./whir-old/` proof-converter only as a workflow reference** for how fixture export is structured. It is not a schema or format reference for the current proof format.
...
- **Typed ABI first.** The first working verifier uses Solidity structs with standard ABI encoding. The Rust exporter produces binary `abi.encode(...)` payloads using the `alloy-sol-types` library. Solidity tests load these via `vm.readFileBinary` and decode with `abi.decode(...)`. The standalone-WHIR verifier also has a dedicated blob format with both a wrapper verifier and a native verifier. Full-Spartan blob support is deferred.
...
- **Standalone WHIR uses fixed-config verifiers only.** Schedule-specific wrappers such as `WhirVerifier4_lir6_ff5_rsv1.sol` and `WhirVerifier8_k22_jb100_lir6_ff4_rsv1.sol` bake the derived WHIR schedule, Fiat-Shamir pattern, and digest configuration into generated Solidity constants. Solidity does not accept an `ExpandedWhirConfig`-style runtime ABI config.
...
All Solidity-facing proof and config structs are defined before the Rust exporter is built. Both sides target the same schema. The Rust exporter and Solidity verifier must agree on these definitions exactly.
...
|   |   |   +-- WhirVerifier4_lir6_ff5_rsv1.sol      # quartic fixed-config wrapper (benchmarks/production)
|   |   |   +-- k22_jb100_lir6_ff4_rsv1/
|   |   |   |   +-- WhirVerifier8_k22_jb100_lir6_ff4_rsv1.sol   # octic fixed-config wrapper
|   |   +-- spartan/
...
|   |   |   +-- SpartanVerifier8_*.sol # octic fixed-config wrapper (generated later in Stage 5)
...
- Isolate performance-critical functions as separate internal functions:
  ...
  - Dot-product accumulation (initially, this can stay inside `_mul_coeffs`. Later, in Stage 4, we can replace the internals of `_mul_coeffs` with a faster delayed-reduction version, or a fused version that avoids repeated unpacking, packing, and temporary intermediates, without changing callers.)
...
- Start this stage with transcript traces generated from the current Rust schedule (`FoldingFactor::Constant(whir.folding_factor)`). The challenger implementation itself is schedule-agnostic; if the schedule is changed later, regenerate the traces for the chosen schedule and rerun the same parity tests.
...
- WHIR domain-separator observation: the Rust exporter emits the exact WHIR Fiat-Shamir pattern for each config. The fixed-config verifiers bake this pattern as a compile-time constant generated from Rust.
...
- `WhirVerifier4_lir6_ff5_rsv1.sol` / `WhirVerifier8_k22_jb100_lir6_ff4_rsv1.sol`: thin wrappers with fixed (compile-time) configuration for gas benchmarks and production use.
...
4. Octic standalone WHIR verifier passes using the same core logic with the schedule-specific `WhirVerifier8_k22_jb100_lir6_ff4_rsv1.sol` family.
...
- Accepted optimizations (continued):
  ...
  - compact commitment parsing on the verifier path: `FixedParsedCommitment` retains only the Merkle root and OOD flat points; OOD evaluation arrays are no longer materialized during verification
  ...
  - fold mod elimination in `_foldOnceWithCoeffs`, `_foldOnce`, and `_foldOnceBase`: removed algebraically redundant `mod` on differences d_i; the differences ∈ [1, 2P-1] are congruent mod P and the final `mod` on each output coefficient handles reduction; products r_j × d_i < P × 2P ≈ 2^63 fit in uint256; saved `11,268` gas across ~22 inlined call sites
  - cascade `_bswap64`: replaced 8-term shift+mask+or byte-swap with 3-step cascade (swap adjacent bytes, swap 16-bit pairs, swap 32-bit halves); saved `384` gas
  - `sampleBitsUnchecked` unchecked mask: wrapped the mask computation `(1 << bits) - 1` in `unchecked`; the `unchecked` context propagates through the inlined `_sampleUint32` callee; saved `513` gas
- Rejected optimizations (continued):
  ...
  - challenger `INITIAL_CAPACITY` increase from 64 to 512: saved only `141` gas with `+8` bytes bytecode; the doubling-array reallocation pattern is already cheap and `duplexing()` resets the buffer frequently
- Rejected optimization for the lifetime-split pass:
  - helper relocation to save bytecode (`_statementFromCalldata`, `_concatenateEq`, `_emptySelect` moved out of `WhirVerifierCore4`): no deployable bytecode reduction and no gas change; reverted
  ...
  - fully generic parser using `_parseFixedCommitment(..., oodSamples = 2)` for the initial commitment and both rounds: increased deployability margin only marginally beyond the fixed-two-sample helper but regressed canonical gas to `1,017,705`; rejected in favor of `_parseFixedCommitment2(...)` as the better bytecode-saving option
  - later parser specialization experiments after the wrapper bytecode cleanup:
    - round-0-only specialization on top of that wrapper state (`_parseFixedCommitment12x2` for round 0): deployable at about `24,455` bytes and lowered canonical gas to `1,007,368`, but was dominated by the initial-only variant and therefore not kept
    - round-1-only specialization on top of that wrapper state (`_parseFixedCommitment8x2` added on top of the round-0-only variant): lowered canonical gas further to `1,006,973` but increased deployed bytecode to about `24,811` bytes, exceeding EIP-170; reverted as non-deployable
...
- Current stage4 state (24 April 2026):
  - `foundry.toml`: `optimizer_runs = 833`
  - `WhirVerifier4.testGasWhirVerifyFixed()`: `992,447`
  - `WhirVerifier4.testVerifyQuarticWhirSuccessFixture()`: `992,293`
  - deployed bytecode from `forge inspect src/whir/lir6/WhirVerifier4_lir6_ff5_rsv1.sol:WhirVerifier4 deployedBytecode | wc -c`: `48,017` hex chars, about `24,008` bytes, so deployable under EIP-170 with about `568` bytes of headroom

#### Current Phase Profile

Measured with `forge test --match-test testProfileFullBreakdown -vv`.
The cross-schedule coverage map for transaction calldata and phase-level gas
rows across the quartic, quintic, and octic verifier families is maintained in
[`sol-spartan-whir/docs/verifier-gas-breakdown-matrix.md`](sol-spartan-whir/docs/verifier-gas-breakdown-matrix.md).

    | Component               |       Gas |
    | ----------------------- | --------: |
    | Setup                   |  `28,130` |
    | Initial sumcheck        |  `21,561` |
    | Round0 parse commitment |  `19,005` |
    | Round0 STIR             | `188,933` |
    | Round0 sumcheck         |  `22,003` |
    | Round1 parse commitment |  `14,493` |
    | Round1 STIR             | `153,012` |
    | Round1 sumcheck         |  `25,523` |
    | Observe final poly      |  `11,383` |
    | Final STIR              | `143,002` |
    | Final sumcheck          |  `21,563` |
    | Constraint evaluation   | `185,448` |
    | Final value check       |  `10,510` |
    | Sum of profiled phases  | `844,566` |

#### Current STIR Breakdown

Measured with `forge test --match-test testProfileStirBreakdown -vv`.

    | STIR phase | `sampleQueries` | `leafHashing` | `merkleReduction` |    `pow` | `rowFolding` | `overhead` |   `total` |
    | ---------- | --------------: | ------------: | ----------------: | -------: | -----------: | ---------: | --------: |
    | Round0     |         `8,990` |      `20,810` |         `176,649` | `18,611` |     `64,602` |    `5,492` | `295,154` |
    | Round1     |         `6,557` |      `21,938` |         `113,527` | `11,939` |     `60,780` |    `4,946` | `219,687` |
    | Final      |         `5,444` |      `18,295` |          `87,234` |  `9,102` |     `50,515` |    `4,403` | `174,993` |

- standalone blob verifier on this branch:
  - success blob fixture (`quartic_whir_lir6_ff5_rsv1_success.blob`): `10,152` bytes
  - wrapper verifier:
    - `WhirBlobVerifier4.testGasWhirVerifyBlobFixed()`: `1,193,829`
    - useful for malformed-blob rejection and calldata-size benchmarking, but not an execution-gas win
  - native blob verifier:
    - `WhirBlobVerifierNative4.testGasWhirVerifyBlobNativeFixed()`: `899,906`
    - `WhirBlobVerifierNative4.testVerifyQuarticWhirSuccessBlobNative()`: `899,884`
    - deployed bytecode from `forge inspect src/whir/lir6/WhirBlobVerifierNative4_lir6_ff5_rsv1.sol:WhirBlobVerifierNative4 deployedBytecode | wc -c`: `43,755` hex chars, about `21,877` bytes, so deployable under EIP-170 with about `2,699` bytes of headroom
    - this is the fastest verifier on this branch and the recommended deployment target, because it verifies directly from the standalone blob instead of materializing typed proof structs
    - current blob layout:
...
- the measured changes below show how the verifier reached the current state
- canonical transcript change in stage4:
  ...
  - measured result right after that change:
    - typed verifier: `1,007,179 -> 1,008,830` (`+1,651`)
    - blob wrapper: `1,190,987 -> 1,194,448` (`+3,461`)
    - native blob: `937,595 -> 937,394` (`-201`)
- lifetime-split cleanup: `1,049,006 -> 1,040,144` (`-8,862`)
  - compact constraint split on the production path: `1,049,006 -> 1,041,352` (`-7,654`)
  - raw frontier allocation in `_computeRootFromFlatRows20`: `1,041,352 -> 1,040,144` (`-1,208`)
- Merkle frontier-loop cleanup: `1,040,144 -> 1,037,139` (`-3,005`)
- parser and raw fixed-select cleanup: `1,037,139 -> 1,022,824` (`-14,315`)
...
- exact-shape initial-constraint cleanup: `1,022,824 -> 1,021,626` (`-1,198`)
  - production-only initial claim accumulation for the fixed statement shape: `_combineInitialConstraintEvalsSingle(...)`
  - production-only initial weight evaluation for the fixed statement shape: `_evaluateInitialConstraintSingle(...)`
- fixed-config verifier simplification: `1,021,626 -> 1,012,306` (`-9,320`)
  ...
  - introduce a fixed-two-sample production parser (`_parseFixedCommitment2`) and use it on the production verifier path instead of the generic `oodSamples` loop: `1,019,799 -> 1,016,253` (`-3,546`)
  ...
  - replace fixed-config round-parameter loads (`powBits`, `foldingFactor`, `nextFoldingFactor`, and `roundConfig(...)` scaffolding that was only selecting constants) with direct constants: `1,015,593 -> 1,013,994` (`-1,599`)
  - fully specialize the fixed parsers by arity (`_parseFixedCommitment16x2`, `_parseFixedCommitment12x2`, `_parseFixedCommitment8x2`) once the earlier size savings created enough bytecode headroom: `1,013,994 -> 1,012,306` (`-1,688`)
- size-for-bytecode tradeoff: `1,012,306 -> 1,014,184` (`+1,878` gas) in exchange for `24,542 -> 23,214` bytes of deployed bytecode (`-1,328` bytes)
  - roll back the fixed parser path from fully arity-specialized wrappers to the fixed-two-sample generic helper (`_parseFixedCommitment2`) for the initial commitment and both rounds
  ...
  - this was the chosen point for further verifier work because it restored over a kilobyte of EIP-170 margin while keeping canonical gas close to the lowest measured gas state
- wrapper simplification: `1,014,184 -> 1,008,499` (`-5,685`)
...
- small wrapper bytecode reduction: `1,008,499 -> 1,007,866` (`-633`) and `24,442 -> 24,129` bytes (`-313`)
  ...
  - keep success-path behavior and transcript flow unchanged; this is only a wrapper bytecode cleanup
- final parser specialization: `1,007,866 -> 1,007,224` (`-642`)
...
- native blob verifier improvements:
  - `960,622 -> 954,852` (`-5,770`)
  - eliminate the per-round `foldingRandomness` arrays in the native blob verifier by writing sumcheck challenges directly into `allRandomness` and reusing those windows for STIR folding
  ...
  - `951,351 -> 938,996` (`-12,355`)
    ...
    - the typed fixed verifier also improved slightly: `1,007,224 -> 1,007,179`
  - `938,996 -> 937,595` (`-1,401`)
    ...
    - this reduces code surface and also improves the compiled native blob verifier: runtime bytecode `21,237 -> 20,216` bytes
...
- transcript-native blob rewrite after the canonical challenger change:
  - rewrite the standalone blob layout in place so the transcript-fed sections that are not Merkle-heavy or arithmetic-hot are emitted in transcript-native canonical little-endian form
  - sections that moved:
  ...
  - the accepted fix was to collapse LE observe + decode + validation back into combined native helpers:
    - `KeccakChallenger.observeReadValidatedPackedExt4Le(...)`
    - `KeccakChallenger.observeReadValidatedPackedExt4LePair(...)`
  - measured result relative to the native verifier right after the canonical challenger change:
    - native blob: `937,394 -> 927,944` (`-9,450`)
    - wrapper blob: `1,194,448 -> 1,210,200` (`+15,752`)
    - typed verifier unchanged from that point
  - accepted follow-up native-only wins on top of that transcript-native state:
    - `927,944 -> 921,702` (`-6,242`)
      - fuse native STIR row hashing and row evaluation so each opened row is loaded once instead of once for Merkle hashing and again for folding
    - `921,702 -> 917,474` (`-4,228`)
      - replace the five separate final STIR `hornerBaseBlob(...)` scans with one fixed 16-coefficient batch checker that keeps scalar lane accumulators in one assembly loop
    - `917,474 -> 915,401` (`-2,073`)
      - raw-allocate the native `frontier` and `rowEvals` buffers in `_computeBaseRootAndEvalsBlob16` and `_computeExtensionRootAndEvalsBlob16` because both buffers are fully overwritten before any read
      - this also trims native runtime bytecode `22,041 -> 21,940` bytes
  - accepted constant-factor optimizations on shared code (affects all verifier paths):
    - `915,401 -> 904,133` (`-11,268`)
      - remove 4 algebraically redundant `mod` operations on differences in `_foldOnceWithCoeffs` and `_foldOnce`, and 1 in `_foldOnceBase`; the differences d_i ∈ [1, 2P-1] are congruent mod P and the final `mod` on each output coefficient handles reduction; saves ~500 gas per inlined call site across ~22 call sites in the native verifier
    - `904,133 -> 903,749` (`-384`)
      - replace the 8-term shift+mask+or `_bswap64` with a 3-step cascade byte-swap (swap adjacent bytes, swap 16-bit pairs, swap 32-bit halves)
    - `903,749 -> 903,236` (`-513`)
      - wrap the mask computation in `sampleBitsUnchecked` in `unchecked`; the `unchecked` context propagates through the inlined `_sampleUint32` callee
    - net constant-factor savings: `915,401 -> 903,236` (`-12,165`, `-1.33%`), native bytecode `21,940 -> 21,889` bytes (`-51`)
  - latest transcript-buffer and ext8 observation cleanup:
    - quartic typed verifier: `996,068 -> 992,447` (`-3,621`)
    - quartic blob wrapper: `1,197,438 -> 1,193,829` (`-3,609`)
    - quartic native blob: `903,236 -> 899,906` (`-3,330`)
    - quartic native total tx gas: `975,292 -> 975,202` (`-90`)
    - octic typed verifier: `8,888,783 -> 8,888,020` (`-763`)
    - octic native blob: `7,865,125 -> 7,861,397` (`-3,728`)
    - octic native total tx gas: `8,146,817 -> 8,143,089` (`-3,728`)
    - replace `new bytes(newCapacity)` in `KeccakChallenger._ensureCapacity(...)` with raw allocation, match the single-value ext8 transcript read path to the already-optimized pair reader, and mark the remaining safe shared assembly blocks as `assembly ("memory-safe")`
    - attempted Solady-style conditional-reduction rewrites for `KoalaBearExt4.add/sub` and `KoalaBearExt8.add/sub` regressed under `via_ir` and were reverted
  - depth-4 equality-monomial row-evaluation rewrite:
    - quintic `rsv4` native blob: `8,461,314 -> 6,733,159` (`-1,728,155`)
    - quintic `rsv4` native total tx gas: `8,551,179 -> 6,822,886` (`-1,728,293`)
    - octic native blob: `7,861,397 -> 7,383,992` (`-477,405`)
    - octic native total tx gas: `8,143,089 -> 7,665,684` (`-477,405`)
    - precompute the 16 equality monomial weights for each depth-4 STIR row evaluation, then evaluate rows as a dot product instead of repeatedly walking the binary fold tree
    - the same rewrite was tested on the quartic `lir6` native verifier and reverted because it regressed gas (`899,906 -> 1,096,553`)
  - lazy-reduction octic row dot products:
    - octic native blob: `7,383,992 -> 6,908,778` (`-475,214`)
    - octic native total tx gas: `7,665,684 -> 7,190,470` (`-475,214`)
    - for extension-row STIR dot products, unpack the 16 precomputed equality weights once and accumulate the raw octic channels before one final reduction
  - lazy-reduction quintic dot products:
    - current quintic `rsv3_pow28` native blob: `5,903,484 -> 5,750,877` (`-152,607`)
    - current quintic `rsv3_pow28` native total tx gas: `6,022,631 -> 5,870,024` (`-152,607`)
    - quintic `rsv4` native blob: `6,733,159 -> 6,583,570` (`-149,589`)
    - quintic `rsv4` native total tx gas: `6,822,886 -> 6,673,297` (`-149,589`)
    - for extension-row STIR dot products, unpack the 16 precomputed equality weights once and accumulate the raw quintic convolution channels before one final reduction
    - base-row lazy dot product:
      - current quintic `rsv3_pow28` native blob: `5,750,877 -> 5,613,844` (`-137,033`)
      - current quintic `rsv3_pow28` native total tx gas: `5,870,024 -> 5,732,991` (`-137,033`)
      - quintic `rsv4` native blob: `6,583,570 -> 6,454,427` (`-129,143`)
      - quintic `rsv4` native total tx gas: `6,673,297 -> 6,544,154` (`-129,143`)
      - keep the packed equality-weight buffer for base rows, but accumulate the 16 scalar-times-ext5-weight products into five raw coefficient sums before one final reduction
    - pairwise constraint-select evaluation:
      - current quintic `rsv3_pow28` native blob: `5,613,844 -> 5,526,933` (`-86,911`)
      - current quintic `rsv3_pow28` native total tx gas: `5,732,991 -> 5,646,080` (`-86,911`)
      - quintic `rsv4` native blob: `6,454,427 -> 6,357,321` (`-97,106`)
      - quintic `rsv4` native total tx gas: `6,544,154 -> 6,447,048` (`-97,106`)
      - evaluate two select terms per loop so both products share each loaded `fullPoint` element
      - the same source-level rewrite was tested on the octic native verifier and reverted because it regressed gas (`7,383,992 -> 7,940,820`)
  - rejected constant-factor candidate:
    - increasing `INITIAL_CAPACITY` in `KeccakChallenger` from 64 to 512: saved only `141` gas with `+8` bytes bytecode; the doubling-array pattern is already cheap and `duplexing()` resets the buffer frequently
  - conclusion:
    - the stage4 native blob verifier benefits materially
    - the wrapper remains a reference / malformed-blob / calldata-benchmark path and is not an execution target
- rejected native-blob experiments after that point:
  ...
  - monolithic assembly rewrite of `_evaluateExtensionRowDim4BlobWindow`: regressed native blob gas to `919,809` and increased native runtime bytecode `22,041 -> 23,101` bytes
- verifier state after that parser work:
  - same as the current state above

- Current focused STIR detail profiling (from `test/WhirStirDetailProfile_lir6_ff5_rsv1.t.sol`):
  - final STIR materialization: `143,026`
  - final STIR check-only: `0`
  - Round 0 frontier: `frontierInit = 22,051`, `merkleLoopOnly = 170,724`
  - Round 1 frontier: `frontierInit = 22,760`, `merkleLoopOnly = 109,421`
  - Final frontier: `frontierInit = 19,026`, `merkleLoopOnly = 84,481`

### Local KoalaBear Ext8 Arithmetic Precompile Work

The Solidity project includes an experimental octic verifier variant that calls custom EVM precompiles for KoalaBear ext8 arithmetic. The precompiles are not part of mainnet Ethereum; they are served by a patched Anvil node vendored under `foundry-b0a9dd9/`, and the verifier variant only works when run against that node. The experiment answers one question: whether a generic EIP-style precompile family for octic extension-field multiplication, squaring, and vector multiply-accumulate could remove meaningful gas from the current octic verifier without changing the proof system.

Detailed runner setup, precompile addresses, calibration procedures, harness commands, and subsystem measurements live in [sol-spartan-whir/README.md](sol-spartan-whir/README.md#precompile-backed-octic-experiment). The root README keeps the cross-workspace conclusion and the headline full-verifier numbers.
...
- Merkle tree layout
- calldata encoding
- the baseline octic verifier source (the one that runs on standard EVM, with no custom precompiles)

Current conclusion:

- `EXT8_MUL` and `EXT8_SQUARE` save enough gas to justify building the precompile-backed verifier variant at all.
- `EXT8_MUL_BATCH` helps in the fixed equality evaluator, where many independent ext8 multiplications can be grouped into a single batch call instead of one `STATICCALL` per multiplication.
- `EXTFIELD_MAC` helps in STIR extension-row evaluation, where each row is a 16-term extension-field dot product. The current precompile-backed verifier uses one `EXTFIELD_MAC(n = 16)` call per extension row instead of the previous fold-tree sequence of singleton ext8 multiplication calls.
- `EXT8_ADD`, `EXT8_SUB`, `EXT8_MUL_BASE`, `EXT8_SQUARE_BATCH`, and `EXT8_MUL_BASE_BATCH` do not produce a measurable net benefit in the integrated verifier and are not used.
- Purpose-built precompiles that fuse multiple WHIR-specific verifier steps into a single call are out of scope. The goal is a general-purpose ext8 precompile family that any verifier over KoalaBear could use, not a Spartan-WHIR-shaped primitive.
- The baseline octic verifier (standard EVM) remains the canonical Solidity verifier. The precompile-backed verifier is a separate comparison build used only to measure what the precompiles would save; it requires the patched Anvil node from `foundry-b0a9dd9/`.

The full-verifier comparison on the patched node is:

| Verifier                               | Total tx gas | Calldata gas | Execution gas |
| -------------------------------------- | ------------ | ------------ | ------------- |
| baseline octic (standard EVM)          | `7,190,470`  | `753,800`    | `6,415,670`   |
| precompile-backed octic (patched node) | `4,345,429`  | `753,800`    | `3,570,629`   |
| savings                                | `2,845,041`  | `0`          | `2,845,041`   |

The Rust precompile implementation calls Plonky3 field types directly:

- `p3_koala_bear::KoalaBear`
- `p3_field::extension::BinomialExtensionField<KoalaBear, 8>`

It intentionally does not depend on the `spartan-whir` workspace crate and does not re-implement ext8 multiplication by hand.

### Local KoalaBear Ext5 Arithmetic Precompile Work

The Solidity project also includes an experimental quintic verifier variant that calls local KoalaBear ext5 precompiles registered with the patched Anvil node. This is the same comparison-only setup as the octic experiment: the baseline quintic verifier remains the canonical standard-EVM path, and the precompile-backed verifier is a parallel `view` variant used to measure scalar, batch, multiply-accumulate, and linear-product extension-field precompile shapes.

Detailed runner setup, precompile addresses, calibration procedures, harness commands, and subsystem measurements live in [sol-spartan-whir/README.md](sol-spartan-whir/README.md#quintic-precompile-experiment).

The experiment does not change WHIR protocol parameters, verifier external ABI, proof blob format, transcript ordering, Merkle tree layout, calldata encoding, or the baseline quintic verifier source.

The full-verifier comparison on the patched node is:

| Verifier                               | Total tx gas |
| -------------------------------------- | ------------ |
| baseline quintic `rsv3_pow28`          | `5,646,080`  |
| precompile-backed quintic `rsv3_pow28` | `4,328,805`  |
| savings                                | `1,317,275`  |

This is useful for the current quintic target and reflects the extension-field vector multiply-accumulate precompile used by the extension-row dot products, plus the pairwise constraint-select evaluation used by both the software and precompile-backed quintic paths. A precompile-backed variant of the older `rsv4` quintic verifier remains a separate port and must be measured independently.

`EXTFIELD_LIN_PROD` was also prototyped for select-product chains as an EIP-style small-prime extension-field primitive. It validated cleanly, but it is not wired into the verifier because the current quintic chain sizes are too small for the transport cost: the weighted projection was `-104,698` gas against the existing software select-product kernels.
...
- `WhirVerifier4.testGasWhirVerifyFixed()` at the highest deployable optimizer point for the new design:
  - earlier compact multiproof result: `1,092,539` at `optimizer_runs = 325`
...
- `WhirVerifier4.testGasWhirVerifyFixed()`: `1,054,301` (before linearization) → `986,923` (`-67,378` / `-6.4%`)
...
- the stage4 standalone blob now uses transcript-native canonical little-endian bytes for:
  - initial/per-round OOD answers
  - all sumcheck polynomial evaluations
  - round/final PoW witnesses
  - round1 sumcheck PoW witnesses

Measured result on the native blob verifier:

- result immediately after the canonical challenger change: `937,394`
- after the accepted transcript-native blob rewrite: `927,944`
- net native blob saving: `-9,450`
- after the later native-only verifier optimizations that kept the transcript-native layout intact: `915,401`
- after constant-factor optimizations on shared code (fold mod elimination, cascade bswap64, unchecked mask): `903,236`
- after the later transcript-buffer and ext8 observation cleanup: `899,906`
...
1. Use the current Rust `FoldingFactor::Constant(whir.folding_factor)` schedule as the starting implementation path through Stages 2-4.
2. After Stage 4 is passing, compare that starting point against candidate schedules on the same statements and security settings. Start with the current `Constant(4)` schedule and, if valid for the chosen statements, `ConstantFromSecondRound(3,4)` as the first candidate. This is not a claim that `(3,4)` is always optimal; it is the first comparison point because it perturbs only the first round while keeping the later-round factor at `4`, which keeps the search space small and isolates whether "smaller first fold, same later folds" helps.
...
Separate thin wrappers: a quartic `SpartanVerifier4...` family and a schedule-specific octic `SpartanVerifier8...` family.

Runtime verifier input: `SpartanInstance` (public inputs + witness commitment) + `SpartanProof`.

**Bytecode-size and deployment-gas check** (required after the first generated version is working): measure runtime bytecode size, initcode size, and deployment gas. Memory-budget profiling (peak memory usage, temporary array sizes) should accompany these measurements.
...
- **Every Stage 6 optimization requires:**
  - Differential tests against the unoptimized starting kernel.
  - Before/after gas measurements on identical fixtures.
...
- Current implemented scope on `stage4`: a quartic standalone blob format for the current 16-variable / 2-round / ext4 proof family, with two Solidity entrypoints:
  - `WhirBlobVerifier4`: decode-and-delegate wrapper over the typed verifier
  - `WhirBlobVerifierNative4`: native verifier that consumes the blob sequentially from calldata
...
- Use Rust-exported real blob fixtures for testing:
  - current `stage4`: standalone WHIR blobs
...
- Keep all relevant entry points available: the typed verifier as the reference path, the standalone wrapper blob verifier as the decode-and-delegate reference for blob correctness, and the standalone native blob verifier as the deployable execution path if it reduces total cost.
...
- **Standalone WHIR** (Stage 4):
  ...
  - Standalone blob parity for both the wrapper verifier and the native blob verifier, plus malformed-blob rejection.
...
- **Gas benchmarks** (Stage 4 and Stages 6-7): benchmarks on fixed fixtures for quartic and octic standalone WHIR, quartic and octic full Spartan, typed ABI vs blob calldata. For standalone WHIR, include the typed ABI path, the standalone blob wrapper verifier, and the standalone blob native verifier.
...
- Also keep one direct transaction benchmark for the fixed verifier using a broadcast script against a local Anvil node, so we record the actual user-paid `EOA -> verify(...)` transaction gas in addition to the in-test call-path benchmark.
...
- Record typed-ABI, blob-wrapper, and blob-native results separately once those entry points exist. For standalone WHIR, this now includes both standalone blob verifiers.

## Assumptions

- First correctness target is **quartic standalone WHIR**.
...
- The typed ABI with `abi.encode`/`abi.decode` is the first correctness path. Standalone WHIR now also has a standalone blob decoding path; full-Spartan blob decoding remains a later calldata optimization.
...
- Current `spartan-whir` wiring uses `FoldingFactor::Constant`, because the public config surface exposes only one `whir.folding_factor` scalar and `build_whir_config` maps that scalar directly to `FoldingFactor::Constant` ([spartan-whir/src/whir_pcs.rs](spartan-whir/src/whir_pcs.rs) line 311). If `ConstantFromSecondRound` or another derived schedule is adopted later, Rust must regenerate the WHIR Fiat-Shamir pattern, proof fixtures, and fixed-config Solidity constants for that schedule. Follow the Schedule Tuning Pass after Stage 4 correctness and gas measurements, and before Stage 5+: compare proof bytes and actual standalone verifier gas, then freeze the schedule and regenerate the affected fixtures and generated artifacts for the chosen schedule.
...
- The WHIR Fiat-Shamir domain-separator pattern length should be recorded in fixture metadata for bytecode-size estimation.
