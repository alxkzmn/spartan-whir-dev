...
- Current linearized-merkle state (11 April 2026):
  - `foundry.toml`: `optimizer_runs = 833`
  - `WhirVerifier4.testGasWhirVerifyFixed()`: `932,563`
  - `WhirVerifier4.testVerifyQuarticWhirSuccessFixture()`: `932,409`
  - deployed bytecode from `forge inspect src/whir/WhirVerifier4.sol:WhirVerifier4 deployedBytecode | wc -c`: `42,876` hex chars, about `21,437` bytes, so deployable under EIP-170 with about `3,139` bytes of headroom
...
Measured with `forge test --match-test testProfileFullBreakdown -vv`.
    ...
    | Round0 parse commitment |  `19,090` |
    | Round0 STIR             | `154,618` |
    | Round0 sumcheck         |  `22,102` |
    | Round1 parse commitment |  `14,576` |
    | Round1 STIR             | `129,955` |
    | Round1 sumcheck         |  `25,602` |
    | Observe final poly      |  `11,387` |
    | Final STIR              | `125,627` |
    | Final sumcheck          |  `21,651` |
    | Constraint evaluation   | `185,437` |
    | Final value check       |  `10,519` |
    | Sum of profiled phases  | `770,784` |
...
Measured with `forge test --match-test testProfileStirBreakdown -vv`.
    ...
    | ---------- | --------------: | ------------: | ----------------: | -------: | -----------: | ---------: | --------: |
    | Round0     |         `8,988` |      `20,591` |          `76,215` | `18,611` |     `64,602` |    `5,485` | `194,492` |
    | Round1     |         `6,556` |      `21,938` |          `48,312` | `11,939` |     `60,780` |    `4,912` | `154,437` |
    | Final      |         `5,439` |      `18,299` |          `37,614` |  `9,117` |     `50,530` |    `4,445` | `125,444` |

- standalone blob verifier on this branch:
  - success blob fixture (`quartic_whir_success.blob`): `11,512` bytes
  - wrapper verifier:
    - `WhirBlobVerifier4.testGasWhirVerifyBlobFixed()`: `1,154,373`
    - useful for malformed-blob rejection and calldata-size benchmarking, but not an execution-gas win
  - native blob verifier:
    - `WhirBlobVerifierNative4.testGasWhirVerifyBlobNativeFixed()`: `842,343`
    - `WhirBlobVerifierNative4.testVerifyQuarticWhirSuccessBlobNative()`: `842,321`
    - deployed bytecode from `forge inspect src/whir/WhirBlobVerifierNative4.sol:WhirBlobVerifierNative4 deployedBytecode | wc -c`: `42,838` hex chars, about `21,418` bytes, so deployable under EIP-170 with about `3,158` bytes of headroom
...
#### 1. Linearized Merkle Authentication Paths (Active — Current Branch)
...
- Current implemented scope on `linearized-merkle`: a quartic standalone blob format for the current 16-variable / 2-round / ext4 proof family, with two Solidity entrypoints:
...
- Use Rust-exported real blob fixtures for testing:
  - current `linearized-merkle`: standalone WHIR blobs
